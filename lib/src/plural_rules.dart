...
int _decimals(num n) {
  final String str = _precision == null ? '$n' : n.toStringAsFixed(_precision!);
  final int result = str.indexOf('.');
...
void _updateVF(num n) {
  const int defaultDigits = 3;

  _v = _precision ?? math.min(_decimals(n), defaultDigits);

  final int base = math.pow(10, _v) as int;
...
final Map<String, PluralCase Function()> pluralRules = <String, PluralCase Function()>{
  'af': _es_rule,
  'am': _hi_rule,
  'ar': _ar_rule,
  'az': _es_rule,
